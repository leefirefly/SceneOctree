...
std::istream &SceneOcTreeNode::readData(std::istream &s)
{
  ...
  //cout << "test Node read Data" << scene.objectData << endl;
  return s;
}
...
SceneOcTree *SceneOcTree::readTree(const std::string &filename)
{
  ...
  else
  {
    ...
    AbstractOcTree *tree0 = SceneOcTree::createTree(id, res);
    SceneOcTree *tree = dynamic_cast<SceneOcTree *>(tree0);
    ...
    while (line.size() != 0)
    {
      ...
      if (SceneOcTree *new_tree = dynamic_cast<SceneOcTree *>(tree))
      {
        for (SceneOcTree::leaf_iterator it = new_tree->begin_leafs(), end = new_tree->end_leafs(); it != end; ++it)
        {
          ...
          cout << "test iterator, find node::" << result->getScene().objectData.actorName << endl;
        }
...
SceneOcTree *SceneOcTree::createTree_new(const std::string class_name, double res)
{
  std::map<std::string, SceneOcTree *>::iterator it = classIDMapping_new().find(class_name);
  if (it == classIDMapping_new().end())
  {
    OCTOMAP_ERROR("Could not create octree of type %s, not in store in classIDMapping\n", class_name.c_str());
    return NULL;
  }
  ...
  {
    SceneOcTree *tree = it->second->create();
...
std::map<std::string, SceneOcTree *> &SceneOcTree::classIDMapping_new()
{
  static std::map<std::string, SceneOcTree *> *map = new std::map<std::string, SceneOcTree *>();
  return *map;
}
...
void SceneOcTree::registerTreeType_new(SceneOcTree *tree)
{
  classIDMapping()[tree->getTreeType()] = tree;
}

SceneOcTreeNode *SceneOcTree::getRobotInfo(const int id, const SceneOcTree* tree)
{
  for (SceneOcTree::leaf_iterator it = tree->begin_leafs(), end = tree->end_leafs(); it != end; ++it)
  {
    ...
    if (result->getScene().objectData.robotID == id)
      return result;
  }
  return NULL;
}

int SceneOcTree::getRobotOfTrees(const int id, const std::string &filename, vector<SceneOcTreeNode *> &nodeVector)
{
  int count = 0;
  ...
  for (vector<AbstractOcTree *>::iterator it = treeV.begin(); it != treeV.end(); ++it)
  {
    ...
    {
      SceneOcTreeNode* result = SceneOcTree::getRobotInfo(id,treefromV);
      if(result != NULL)
      {
...
{
  return out << "(Scene: " << (unsigned int)c.r << " " << (unsigned int)c.g << " " << (unsigned int)c.b << " objectData: " << c.objectData << " )";
}
